...
def permcheck(func):
    ...
    def inner(**kwargs):
            ...
            for p in path.split(os.path.pathsep):
                ...
                if os.path.exists(bin_path):
                    ...
                    if os.access(bin_path, os.X_OK):
                        ...
                        logging.error(
                            "You do not have permisison to manage Docker")
        ...
        if not os.path.exists(socket_path):
            logging.error(
                "Docker daemon not found, try starting the Docker service")
            raise FileNotFoundError("Docker daemon not found")
        docker_mode = os.stat(socket_path).st_mode
        isSocket = stat.S_ISSOCK(docker_mode)
        if not isSocket:
            logging.error(
                "Docker daemon is currently not running, try starting the Docker service")
    ...
    def inner(**kwargs):
        try:
            if client.info()['Swarm']['LocalNodeState'] != 'inactive':
                ...
                logging.warning(
                    "Docker not in swarm mode. Would you like to enable swarm mode? (y/n) [n]")
                answer = input().lower()
...
def swarm_init():
    ...
    try:
        ...
        client.swarm.init(
            name="default", advertise_addr=ip_addr, listen_addr=ip_addr)
