...
from ...modules.base import DOCKER_DIR, CONFIG_DIR, get_project_root, \
    git_log, load_yaml_file, bash_command, setup_logger
...
@click.option('--force',
              is_flag=True,
              default=False,
              help='Force full rebuild without using cache.'
              )
@click.option('--push',
              is_flag=True,
              default=False,
              help='Push image to Dockerhub (must be logged-in).'
              )
@click.option('--dryrun',
              is_flag=True,
              default=False,
              help='Only list build command and don\'t actually build.'
              )
        ...
        for dir in os.listdir(folder):
            ...
            if os.path.exists(os.path.join(project_root, DOCKER_DIR, dir, 'Dockerfile')):
                ...
                config_file = os.path.join(
                    project_root, CONFIG_DIR, dir+'.yaml')
                ...
                logging.info("Building the %simage.", tag)
                ...
                if not dryrun:
                    ...
                    if push:
                        ...
                        logging.info('Pushing the %simage.', tag)
                        cmd = 'docker push '+tag
                        bash_command("Pushing container: ", cmd)
                else:
                    logging.info("Build command: %s", cmd)
