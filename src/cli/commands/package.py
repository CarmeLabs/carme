...
from ...modules.base import setup_logger, get_project_root, CONFIG_DIR, CONFIG_FILE, PACKAGE_INDEX, PACKAGE_INDEX_KEY, load_yaml_file, load_yaml_url
...
def _return_package_url(project_root, package):
    ...
    if os.path.exists(config_file):
        ...
        logging.info(
            "Using package_index set in carme-config.yaml : "+index_url)
    else:
    ...
    if package in index:
        ...
        return package
    elif str(package)[0:4] == 'http':
        return package
    else:
        logging.error("The package "+package +
                      " was neither a valid url or contained in the index: "+index_url)
        exit()
