...
from ...modules.base import setup_logger, CWD, CONFIG_DIR, COMMANDS_DIR, get_project_root
from ...modules.commandswrapper import get_project_commands, execute
from ...modules.yamltools import load_yaml_file
import ruamel.yaml
...
def cmd(package, command, docker, dryrun, server, yes):
    ...
    if project_root != CWD:
        logging.info(
            "All cmd commands issued from project root directory to ensure relative path consistency.")
        os.chdir(project_root)
    # Finds the project root directory.
    project_root = get_project_root()
    # Loads the configuration in the root directory.
    package_file = package+'.yaml'
    kwargs = load_yaml_file(os.path.join(
        project_root, CONFIG_DIR, package_file))

    # Loads the commands available in the commands directory.
    commands = load_yaml_file(os.path.join(
        project_root, COMMANDS_DIR, package_file))
    # The list command is used to show availabe commands.
    if command == 'list':
        logging.info('These commands are currently installed:')
        ruamel.yaml.dump(commands, sys.stdout,
    ...
    # This check to see if there are a series of commands passed as a list (example: [com1, com2, com3]).
        ...
        for x in commands[command]:
            execute(x, commands, package, project_root,
                    kwargs, docker, dryrun, yes)

    # This attempts to execute a single command.
    else:
        execute(command, commands, package, project_root,
                kwargs, docker, dryrun, yes)
...
def validate_command(ctx, param, value):
    ...
    if value != 'list' and value not in commands:
        raise logging.warning(
            'This is not a valid command. Use *list* to show installed commands.')
    return value
